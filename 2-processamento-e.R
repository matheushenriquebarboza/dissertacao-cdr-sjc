...
#o arquivo de antenas terá então todas as antenas de sp, além das de fora que não tiverem repetição de ID
IDs_SP <- antenas %>%
  group_by(CELLID) %>%
  filter(UF=="SP") %>% 
...
saveRDS(object = dados, file = "dados.rds") 

#1.1.2 Tratamento dos dados de CDR####
#1.1.2.1 resumo dos dados####
#total
nrow(dados)#39.578.058
nrow(dados %>% group_by(userID) %>% summarise())#445.666
nrow(dados %>% group_by(CELLID) %>% summarise())#1.281

#por dia
cdr_dia <- dados %>%
  group_by(data) %>% 
  summarise(contagem=n(),
            contagem_lig=n_distinct(userID),
            contagem_cell=n_distinct(CELLID))

min(cdr_dia$contagem)#871.072
max(cdr_dia$contagem)#1.598.246
mean(cdr_dia$contagem)#1.319.269
sd(cdr_dia$contagem)#196.464

min(cdr_dia$contagem_lig)#209.675
max(cdr_dia$contagem_lig)#285.673
mean(cdr_dia$contagem_lig)#260.935
sd(cdr_dia$contagem_lig)#18.722

min(cdr_dia$contagem_cell)#1.207
max(cdr_dia$contagem_cell)#1.239
mean(cdr_dia$contagem_cell)#1.223
sd(cdr_dia$contagem_cell)#7,9

#1.1.2.2 distribuição usuarios por numero de chamadas####
...
#graficos distribuicao usuario chamadas
...
#1.1.2.3 define limites inferior e superior de número de ligações feitas####
cmin  <- 6
cmax <- 500
...
#reduz dados para os que aparecem no limite de ligações
dados_usuarios <- dados %>%
  inner_join(ligadores_significativos)

#1.1.2.4 distribuicao locais####
...
#2.1.1.2 registra ligacoes em horarios potencialmente domicilio (código volta pro cdr)####
...
#ITERAÇÃO EXTRA - alguns grupos foram unidos, para que nenhum grupo tivesse menos de 300 usuários detectados com domicilio no grupo
...
#A- caso em que local de maior permanencia é unico####
...
#C - local de maior permanencia não é unico, dentre eles o de n-permanencia não é unico####
...
#para ser viagem usuarios das viagens sequenciais precisam ser o mesmo. também precisa que grupos sejam diferentes, para que haja movimentação
...
#campo criada será utilizado para marcar viagens de base domiciliar extras, por enquanto todas são F
viagens0$criada <- F
...
#2.3.5 adiciona viagem de saída de casa para o conjunto (PD_saiu - L1)
...
#2.3.6 adiciona viagem de chegada em casa para o conjunto (PD_saiu-L2)
...
#hora de chegada, e consegquentemente duração, não foram estimadas
...
#calcula fator f1
grupos$f1 <- grupos$popu / grupos$moradores

colnames(grupos) <- c("grupo","grupo0","moradores_identif","popu","geometry","f1")

#avalia fatores
ggplot(grupos, aes(f1))+
  geom_histogram() #tem valores muuuito altos

ggplot(grupos[grupos$f1<=1000,], aes(f1))+ #histograma para valores menores que 1000
  geom_histogram() 

quantil <- quantile(grupos$f1,probs = seq(0, 1, 0.025), na.rm = TRUE)
...
grupos_resumo <- grupos[,c("grupo","f1")]
...
#2.4.4.3 fator por individuo f2####
...
viagens$f2 <- 1/viagens$ndias

#expande viagens
viagens$f3 <- viagens$f2 * viagens$f1
...
matriz_hora_sjc <- viagens_sjc %>% 
  filter(utilizadas==T) %>%
  group_by(grupo_i, grupo_j, hora_i_exata, criada ) %>%
  summarise( viagens_exp_sjc = sum(f3, na.rm=T) )
...
matriz_hora_sjc_i <- viagens_sjc %>% 
  ...
  group_by(grupo_i, grupo_j, hora_i_exata, criada ) %>%
  summarise( viagens_exp_sjc_i = sum(f3, na.rm=T) )
...
#2.4.5.2.1.1.1 dividindo em 3 partes sem considerar vizinhos
  ...
  matriz_hora_sjc_i_OriDom <- viagens_sjc %>% 
    ...
    group_by(grupo_i, grupo_j, hora_i_exata, criada ) %>%
    summarise( viagens_exp_sjc_i_OriDom = sum(f3, na.rm=T) )
  ...
  matriz_hora_sjc_i_DestDom <- viagens_sjc %>% 
    ...
    group_by(grupo_i, grupo_j, hora_i_exata, criada ) %>%
    summarise( viagens_exp_sjc_i_DestDom = sum(f3, na.rm=T) )
  ...
  matriz_hora_sjc_i_SemDom <- viagens_sjc %>% 
    ...
    group_by(grupo_i, grupo_j, hora_i_exata, criada ) %>%
    summarise( viagens_exp_sjc_i_SemDom = sum(f3, na.rm=T) )
...
#2.4.5.2.1.1.2.2 reclassifica viagens quanto à base domiciliar considerando vizinhos
  ...
  matriz_hora_sjc_i_OriDom_Cviz <- viagens_sjc_SemDom %>%
    ...
    group_by(grupo_i, grupo_j, hora_i_exata, criada ) %>%
    summarise( viagens_exp_sjc_i_OriDom_Cviz = sum(f3, na.rm=T) )
  ...
  matriz_hora_sjc_i_DestDom_Cviz <- viagens_sjc_SemDom %>%
    ...
    group_by(grupo_i, grupo_j, hora_i_exata, criada ) %>%
    summarise( viagens_exp_sjc_i_DestDom_Cviz = sum(f3, na.rm=T) )
  ...
  matriz_hora_sjc_i_SemDom_Cviz <- viagens_sjc_SemDom %>%
    ...
    group_by(grupo_i, grupo_j, hora_i_exata, criada ) %>%
    summarise( viagens_exp_sjc_i_SemDom_Cviz = sum(f3, na.rm=T) )
...
#atribui peso 1 para quadrados pequenos e 25 para grandes - peso proporcional à área
...
temp_plot= ggplot()+
  geom_sf(data=munic_prox, fill=NA, size=1)+
  geom_sf(data=sjc, color='red', fill="gray", size=1.3)+
  geom_sf_label(data=munic_prox, aes(label=name_muni))+
  geom_sf(data=grupos, aes(color="antenas"), show.legend = "point")+
  scale_color_manual(
    values = c("antenas" = "black"), 
    labels = c("antenas" = "Antena"),
    name = ""
  )+
  coord_sf(xlim=c(-46.11,-45.7), ylim=c(-23.31,-22.83))+
...
ggsave(temp_plot, file='!figuras/mapa_antenas_sjc.png')
